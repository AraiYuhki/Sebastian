...
- **実行履歴**：過去の実行を成否つきで一覧表示（自動更新）
- **設定の編集**：`.sebastian-ci.yaml` を画面上で編集し、「保存して検証」で内容チェック
- **実行**：「実行する」ボタンでパイプラインを起動し、**ログがリアルタイムに流れる**
...
```bash
sebastian-ci agent /path/to/workspace --port 8771
```
...
- `agent` を指定したジョブは、そのエージェントへHTTPで送られ、**エージェント側のコンテナで実行**されます。出力はマスターのログにも表示されます。
- `agent` 未指定のジョブは従来どおりローカルで実行され、両者は同じパイプライン内で混在できます。
- エージェントに接続できない・ジョブが失敗した場合は、マスター側で失敗として扱われます。
- エージェントは `GET /agent/info` で使用エンジンとリソース状況を返します。
- 現状のエージェントは**自身のワークスペース**でジョブを実行します（各エージェントに対象リポジトリを配置しておく運用）。
...
| `agent [パス] [--port N]` | 分散実行のエージェント（ジョブ受け付け）を起動する |
